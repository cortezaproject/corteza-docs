...
This rule Record will not be saved when a duplicate value is detected in the selected fields in any existing record of this Module.
...
Click on the specific field under *available fields* to select. Once a field is selected, it moves to the left under the *selected field* column, and vice-versa is true when unselecting the selected fields
