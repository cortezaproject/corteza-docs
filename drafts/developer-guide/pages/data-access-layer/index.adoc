...
Connections and modules, in the context of DAL, can report errors in a special way.
Issues are used to allow modules/connections to be misconfigured while allowing the system to continue working (for example, if you restart the server with a misconfigured connection, it will still start up). It does, however, prevent specific operations such as creating records for a broken module configuration.
