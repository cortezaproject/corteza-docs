...
[NOTE]
====
The two nodes must go through xref:ROOT:developer-guide/federation/node-pair.adoc[node pairing] before they can perform any syncing.
====
...
This can be as simple as allowing access to specific modules and as complex as allowing access to only specific module fields (configured in the xref:ROOT:administrator-guide/federation/index.adoc#expose-module[{PRODUCT_NAME} {APP_NAME_COMPOSE} administration panel], or via the API).
