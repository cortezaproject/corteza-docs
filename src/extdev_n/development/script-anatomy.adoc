...
Ocurred errors are visible in the {app_automation} logs.
This allows for easier debugging.
====

== Meta

Meta properties define the execution context, such as the resource and event type, security context and some meta data such as a label and a description.

=== Parameters

`name`::
    Automaton script's name, used for referencing during execution.
    This is automatically generated so it should be omitted,
