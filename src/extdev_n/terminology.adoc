...
Script or Automation Script::
    Combination triggers, meta data and the code it self,
...
Resource Type::
    ...
    A resource type also defines the input into the automation script.
...
Implicitly triggered scripts::
    Automation scripts that are executed implicitly as a collateral to an event, such as record creation,
