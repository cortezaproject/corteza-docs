...
* SMTP server settings can now be changed via settings and without the server restart ({SERVER_COMMIT_BASE}0b69d1a2[`0b69d1a2`], {WEBAPP_ADMIN_COMMIT_BASE}20a85d8[`20a85d8`]).
* Support interaction with action log (search, record) from workflows; ACTIONLOG_ENABLE_WORKFLOW_FUNCTIONS needs to be set to true ({SERVER_COMMIT_BASE}1014f53a[`1014f53a`]).
* Support for server plugins ({SERVER_COMMIT_BASE}614d2b30[`614d2b30`]).

//
//

.[#2021_9_2-changed]#<<2021_9_2-changed,Changed:>>#
* Compose module fields do not accept reserved system names anymore: `recordID`, `ownedBy`, `createdBy`, `createdAt`, `updatedBy`, `updatedAt`, `deletedBy`, `deletedAt` ({SERVER_COMMIT_BASE}20757e58[`20757e58`], {WEBAPP_ADMIN_COMMIT_BASE}20a85d8[`20a85d8`]).
* Resource translations no longer fallback to base language in case of a missing translation ({SERVER_COMMIT_BASE}4cd54a58[`4cd54a58`]).
...
* Fix on-boot workflow initialization that caused server to crash on start if workflows with invalid trigger configurations were enabled ({SERVER_COMMIT_BASE}415982c8[`415982c8`]).
* Boolean labels on compose not showing properly when value was false ({WEBAPP_COMPOSE_COMMIT_BASE}0330e31[`0330e31`], {WEBAPP_JS_COMMIT_BASE}aef1a14[`aef1a14`]).
* Workflow step configurator sometimes not saving properly if sidebar was opened ({WEBAPP_WORKFLOW_COMMIT_BASE}6d8796e[`6d8796e`]).
* Accent & HTML escaping in translated strings ({SERVER_COMMIT_BASE}556ffc5e[`556ffc5e`]).
* Fixes issues with resource translations (current language, accents, escaped HTML)  ({webapp_compose_commit_base}05178c2b[`05178c2b`]).
* Hide pages if the parent page is marked as not visible ({WEBAPP_COMPOSE_COMMIT_BASE}957a9de2[`957a9de2`]).
* Fix broken permission setting from Compose admin on Module for fields & records ({WEBAPP_COMPOSE_COMMIT_BASE}8ae2a48d[`8ae2a48d`]).
* Fix RenderOptions expr value assignment via selectors ({SERVER_COMMIT_BASE}556ffc5e[`445f0ed5`]).
* RBAC rule migration crash on duplicate rules ({SERVER_COMMIT_BASE}e8bc6141[`e8bc6141`]).
