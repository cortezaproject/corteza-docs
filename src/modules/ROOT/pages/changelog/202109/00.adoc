...
The facility enables you to easily define new API endpoints for webhooks or custom integrations required by your business processes.
...
The authentication client's user interface now provides a series of cURL examples for interacting with authentication clients ({WEBAPP_ADMIN_COMMIT_BASE}16ae4c22[`16ae4c22`]).
//
...
{PRODUCT_NAME} now defines a series of system users and roles that are used for system tasks, such as provisioning and federation.
//
