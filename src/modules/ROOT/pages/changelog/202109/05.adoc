...
* Improve slow performance in some cases with wf-loops by increasing workflow session state flushing from 10 to 1000 ({SERVER_COMMIT_BASE}486a5752[`486a5752`]).
* Prevent table display elements from crashing if an empty join frame is received ({WEBAPP_REPORTER_COMMIT_BASE}f244b7b2[`f244b7b2`]).
* Fixed improper report route bindings ({WEBAPP_REPORTER_COMMIT_BASE}d250827b[`d250827b`]).
* Added missing type definition to the report filter parser registry ({SERVER_COMMIT_BASE}08ef5ab10[`08ef5ab10`]).
