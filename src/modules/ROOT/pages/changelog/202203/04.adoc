...
* Added a new link to parent option to the `Record` module field which dictates whether the input value will be pre-filled with the parent's record ID.
The feature was added to allow some cases from not pre-populating the input, such as a module with a record field that points to itself ({LOCALE_COMMIT_BASE}059ebf8[`059ebf8`], {WEBAPP_COMPOSE_COMMIT_BASE}a79a852[`a79a852`], {WEBAPP_JS_COMMIT_BASE}0e9de63[`0e9de63`]).
...
The change was made due to unexpected cases where filter A resulted after filter B, so the record list showed data from filter A instead of filter B.
...
Previously, the field would show the zero value for that type, such as `0` for the `Number` field.
...
Previously, users could reset their current password to the same one as they've previously used, but now such a change is no longer allowed and caused the password reset to fail ({SERVER_COMMIT_BASE}b44024f[`b44024f`])
...
The `Boolean` field was removed because the truthy/falsey evaluation produced too loose filters, causing unexpected results.
...
The migration step now skips any RBAC rules that would cause a duplicate error when migrated ({SERVER_COMMIT_BASE}1f657b3[`1f657b3`]).
...
The minimum password length is now limited to a minimum of 8 characters, and the other properties no longer allow negative values ({WEBAPP_ADMIN_COMMIT_BASE}192bc08[`192bc08`], {LOCALE_COMMIT_BASE}e0f912b[`e0f912b`], {SERVER_COMMIT_BASE}9b424a6[`9b424a6`]).
...
The table column now properly handles these edge cases and constructs the table appropriately ({WEBAPP_REPORTER_COMMIT_BASE}953556a[`953556a`])
...
The fix changes the used data type for the cases that caused the index to fail.
