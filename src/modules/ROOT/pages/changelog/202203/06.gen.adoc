...
The fix was made by adding a nil check for Vars methods, so it does not cause any unexpected error, if a nil value is used
({SERVER_COMMIT_BASE}9ad5b36[`9ad5b36`]).
* Fixed getting stuck on sign up page on error.
The fix was made by redirecting back to the sign-up page with the respective error message
