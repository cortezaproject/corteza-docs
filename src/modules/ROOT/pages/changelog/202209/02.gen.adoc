...
If your configuration uses reports that utilize multi-value fields, skip this patch version..
====
...
The fix was made by validating duplication only for the selected field
...
The same issue was addressed for record export, but with the module name.
...
In the latter case, the user-provided configuration was never displayed, instead of the default values
...
If you wanted to select a record, you received the error message: `Could not create record: check error messages on the form`, and on the form, it was saying: `record not found`
This fix ensures the proper module ID is used (one from the module field options)when doing lookup.
The fix was made by using the module ID from the module field options and not the field's module ID
...
Filtering sub-workflows with subWorkflow `2 flag` in workflowList was not returning any sub-workflows.
The fix was made by properly applying conditions when the sub-workflow filter was used
...
The fix was made by renaming the file name for resources as per the expected format (kebab-case)
...
The fix was made by short-circuiting processing sooner when totals and page cursor navigation were not required
...
The fix was made by correctly detecting changes in the geometry value since it uses a more complex structure than the rest of the inputs
...
The fix was made by adding an evaluation to the record export filter to correctly evaluate and interpolate placeholders
...
The fix was made by adding a hotfix to ensure that these exceptions are properly encoded and presented to {APP_NAME_COMPOSE} charts
...
The fix was made by re-fetching the external auth providers after updating
...
The fix was made. Translations were added in the locale and applied in admin
...
The fix was made by adding exceptions to reject default configuration options (no connection params) and properly handling the default state on the web application's connection editor
...
The fix was made by adding 'white-space': 'pre-wrap' to the String viewer and preserveWhitespace: 'full' to CRichTextInput parse options
...
The fix was made by improving record iterator implementation for page navigation and sorting, and also fixing store drivers to be consistent with respect to their query language for dateTime, number, and boolean data types
...
The fix was made by introducing new operators `IN` and `NOT IN`, and should be used when filtering records containing a value inside a multi-value field
