...
The change was added to provide more flexibility for the compose pre-filters
...
The change was made because the reordering of fields while searching was causing issues.
...
The fix was made by adding a custom val handler for the bool type to convert it properly while parsing the query according to supported drivers
...
The fix was made by making sure the resource ID will show in the permissions modal title if no other identifiable information exists
