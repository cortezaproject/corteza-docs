...
The change was made by changing the internal logic to rely on parameter presence rather than value
...
When we had a couple of calendars with records or events configured, the event configured calendars were hidden, and no info was displayed.
The fix was made by replacing the throttle with async/await in the CalendarBase component
...
The fix was made by adding a custom val handler for the bool type to convert it properly while parsing the query to the MySQL driver
