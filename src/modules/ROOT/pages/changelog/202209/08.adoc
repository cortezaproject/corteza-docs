...
The change was added to make reminders more user-friendly and easier to use by having the option to set a reminder for more than 24h and to have the option, to link a record to a reminder, and also to un-dismiss a reminder if needed
...
The fix was made by applying the save when the save button is pressed on the data source modal
({CORTEZA_PULL_BASE}889[`889`]).
* Fixed the record list filter not working when field name was having hyphen in the field name.
The fix was made by adding a hyphen to the allowed list while parsing the query
