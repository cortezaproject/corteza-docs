...
The change was added to simplify the styles of the auth pages without the need to alter the source templates
Uploading the image will update the Auth background image, and the styles will supplement the Auth screen's styles
({CORTEZA_PULL_BASE}446[`446`]).
* Added support for opening a record in a new tab or in a modal.
The change was added to make editing of the records a bit more intuitive, since the user will remain on the record list page while making the changes
({CORTEZA_PULL_BASE}524[`524`], {CORTEZA_PULL_BASE}524[`524`]).
* Added support for cloning and coping page blocks in page builders.
The change was added to allow users to duplicate and reuse page blocks more easily
...
The change was added to have the ability to show one or two levels of dynamic and static navigation
({CORTEZA_COMMIT_BASE}41f1283[`41f1283`]).
* Added support for conditional fields.
The change was added to give users more control over how records are displayed
...
The change was added to allow users to go over all their leads one by one without leaving the record page
({CORTEZA_PULL_BASE}836[`836`]).
* Added support for bulk record updates.
The change was added to allow users to update multiple records in a record list
...
The change was added to allow users to quickly apply a filter to the record list
...
The change was added to give the user the ability to view the referenced record selector on the initial record page
...
The change was added because we wanted to provide the option of disabling built-in navigation in favour of redefining it with the new navigation page block
...
That icon is clickable even though a refresh will happen automatically based on the duration you have set.
...
The change was added to enable users to have a better visual representation of their data
...
The change was added to provide users with more customization and styling options on viewing images
({CORTEZA_PULL_BASE}533[`533`]).
* Added an option to magnify certain page blocks when viewing a public {APP_NAME_COMPOSE} page.
A select dropdown has been added in the general tab of the page block configuration modal for calendar, chart, content, comments, file, iframe, record list, report, and record revisions page blocks.
The user can select between Fullwidth or Modal select option, and when enabled, the user will see a magnifying glass icon at the top of the block.
The change was added to help users find the info they are looking for much faster and easier, with the option to magnify certain page blocks
({CORTEZA_PULL_BASE}576[`576`]).
* Added a spinner to indicate when an action in template editor is submitted.
The change was added to have UI consistency, because it was difficult to tell when your changes were being submitted
...
The change was added to give an endpoint response greater extensibility to the integration gateway, as different data can be returned
...
The change was added to extend record filtering support for range filtering of numeric and date-time for a single value field
...
The change was added to extend support for record duplicate detection with multiple fields in a single rule with various detection algorithms and multi-value matching criteria
...
The change was added to center the images on the filebase for a better user experience
...
The change was made by adding an empty table with the `No record revisions found` message in the record page builder
({CORTEZA_PULL_BASE}758[`758`]).
* Changed the Record ID field in module connection settings being able to be disabled.
The change was made to prevent users from disabling the Record ID field on system field mapping and to not show system fields that are not available.
Also, to allow users to easily enable or disable system fields based on their configuration.
The change was made by updating the system view configurator and system field mapping logic to prevent Record ID from being disabled
...
The change was made by filtering how many times a certain page with the same name already exists and including 'copy of' prefix and number suffix
...
* Changed the geometry multi-value map opening the default map location regardless which record location you press.
Now the opened map changes according to the record's geo location.
The change was made to prevent confusion when different markers in multi-value fields were being opened on the map.
This change aims to improve ux and usability of the geometry field.
The change was made Leaflet map instance, and the zoom level was updated
...
The change was made by updating the chart configurations to adjust the existing rotate logic for the above-mentioned charts
({CORTEZA_PULL_BASE}992[`992`]).
* Changed the output format to become hidden if output relative value is selected in date field.
The change was made to prevent confusion since when you have a relative value, the output format is not taken into account.
...
The change was made to prevent users from getting stuck in a loop and to allow them to go back to the previous page that they were onto
...
The fix was made by adding minimum string match criteria to match the exact string match for search results
...
The fix was made by adding a post-processing method to the Gauge Chart {APP_NAME_COMPOSE} type
...
The fix was made by fixing the way filtering works on the integration routes search
...
The fix was made by changing the part of the application programming interface gateway service that handles route updates
