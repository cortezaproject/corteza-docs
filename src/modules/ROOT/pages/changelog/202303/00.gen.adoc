...
The change was added to extend support for record duplicate detection with multiple fields in single rule with various detection algorithm and multi-value matching criteria
...
The change was made to prevent a confusion when different markers in multi-value fields were being opened on the map.
