...
Based on the configuration, it can either filter an existing record list on the page, which shows the records from the same module as the metric, or open a new record list in a modal.
...
The change was added to give users more freedom in defining error handling, and to know where and on which step the error occurred
...
The change was added to make it easier for users to define URLs by leaving out the http:// part
...
The change was made by moving it to per per-page block instead of per field, as it was previously
...
The change was made to improve the accessibility of the search bar and the buttons for the public and admin page.
...
The issue was caused because the SQL Server driver does not yet support offloading of aggregation steps.
...
The fix was made by skipping that provision on SQL Server because it does not need to exist for that driver
...
The fix was made by updating the date and time module field's responsiveness
...
The fix was made by changing the cursor from a text selector to pointer
...
The fix was made by removing the sort on the label and description field
({CORTEZA_PULL_BASE}1176[`1176`]).
