...
The change was added to give users more configuration options when selecting the color or background color for an element
...
The change was made to give developers the option to configure SMTP settings not only from the UI but also through the `env` file
...
The parameter was expecting the incorrect type of value
({CORTEZA_PULL_BASE}1141[`1141`]).
* Fixed copy of page layout in {APP_NAME_COMPOSE}
A page with a layout couldn't be copied because of a duplicate page handle value
