...
The fix was made by allowing alterations to be logged for the default schema, but continuing to block them from being applied
...
* Fixed select inputs that were using the api for search (User, Workflow) didn't work properly
...
This was done by upgrading packages to more stable versions and removing unused dependencies, causing vulnerabilities
