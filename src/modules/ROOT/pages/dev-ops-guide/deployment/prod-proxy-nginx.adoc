...
Following the xref:ROOT:dev-ops-guide/deployment/demo-in-mem.adoc[in-memory demo] or xref:ROOT:dev-ops-guide/deployment/demo-mysql.adoc[demo with MySQL] are enough to get a local instance up and running.
====
...
[source,yaml]
----
include::ROOT:example$docker-compose/prod-proxy-nginx/docker-compose.yaml[]
----

.`custom.conf`
[source]
----
include::ROOT:example$docker-compose/prod-proxy-nginx/custom.conf[]
----
...
[source,shell]
----
include::ROOT:example$docker-compose/prod-proxy-nginx/output-ps.txt[]
----
