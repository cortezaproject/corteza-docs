...
[#dev-ops-guide:setup-production]
= Production setup
...
[IMPORTANT]
====
This example describes a production-ready deployment.
It depends on running nginx-proxy service (see <<dev-ops-guide:nginx-proxy,Nginx Proxy>>).
====
...
[NOTE]
====
If nginx-proxy refuses to start, inspect your configuration files and compare them with ones provided in <<dev-ops-guide:nginx-proxy,Nginx Proxy>> section.
====
