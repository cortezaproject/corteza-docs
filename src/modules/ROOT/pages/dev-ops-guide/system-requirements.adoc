...
The documentation provides configurations guides for https://github.com/jwilder/nginx-proxy[Nginx Reverse Proxy] and the https://github.com/JrCs/docker-letsencrypt-nginx-proxy-companion[LetsEncrypt nginx-proxy companion].
...
We recommend the https://hub.docker.com/_/percona[Percona Server for MySQL] 8.x, either native on the host or provided via Docker.
...
|Medium volume
| *{APP_NAME_MESSAGING}*: sending text messages, links and occasionally sending documents and images; *{APP_NAME_COMPOSE}*: attaching documents to records.
| 25 MB/user/month

| High volume
| *{APP_NAME_MESSAGING}*: uploading large files daily; *{APP_NAME_COMPOSE}*: attaching a high number of documents to records daily.
| 100 MB/user/month
...
Should you enable federation, the nodes must be visible and allow traffic between them on the designated HTTP ports.
...
[NOTE]
====
If you aren't using any other HTTP server, we highly recommend you use https://github.com/jwilder/nginx-proxy[Nginx Reverse Proxy] and https://github.com/JrCs/docker-letsencrypt-nginx-proxy-companion[LetsEncrypt companion container for nginx-prox].
