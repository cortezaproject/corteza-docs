...
[NOTE]
====
The two nodes must go through xref:ROOT:developer-guide/federation/node-pair.adoc[node pairing] before they can perform any node syncing.
====
...
This can be as simple as allowing access to specific modules and as complex as allowing access to only specific module fields (configured in the xref:ROOT:administrator-guide/federation/index.adoc#@TODO[{PRODUCT_NAME} {APP_NAME_COMPOSE} administration panel], or via the API).
...
In our case, we sync {APP_NAME_COMPOSE} records.
