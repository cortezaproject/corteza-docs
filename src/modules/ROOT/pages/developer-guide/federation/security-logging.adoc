...
This enables us to *speed up the development process*, *reduce potential security holes*, and *leverage our RBAC access control facility*.
...
*After the nodes are paired* and the system users are created, the *origin node* can define *access control rules* that prevent the destination node from accessing information it is not allowed to access.
