...
Public channels::
    are open to everyone who is signed up to {APP_NAME_MESSAGING}.
...
Private channels::
    are for discussions that are limited to only selected team members.
    Private Channels are identified by a lock icon.

Direct messages::
    are discussions between two or more people.
...
Choose a channel you want to join and click on the "Join channel" button on the very bottom (covering the {APP_NAME_MESSAGING} input).
