...
Record page::
    *Record pages* are used when *viewing specific records*.
    A record page *is required*, if you want to *interact with a module*.
...
After you save the page, the option becomes available.
====
...
The *page order* (the *nesting* also) defines the *navigation structure*.
