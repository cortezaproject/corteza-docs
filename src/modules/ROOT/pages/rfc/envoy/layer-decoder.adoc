...
It would be a good idea to mention the concept behind it in the developer guide.
====
...
Nothing is stopping us from defining custom decoders for anything we might want to have in the future.
====

The decoder layer is *only allowed* to operate over the *resources* that are *available* in the provided *input source*.
The decoder decodes the available data.
If a thing refers to another thing that is *not* in the given input source, the decoder is *not allowed* to access any other input source to attempt to find it.
