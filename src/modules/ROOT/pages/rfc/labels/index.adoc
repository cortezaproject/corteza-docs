...
* resource grouping without the need for messy prefixes,
...
[NOTE]
====
Should we also constraint the label value?
====

== Label usage

Labels can be used *by any {PRODUCT_NAME} resource* as long as it defines the proper facility for *assigning labels* and *label based retrieving*.
...
We define a *simple generic* store definition for storing labels.
This simplification allows us to use these labels *on any {PRODUCT_NAME} resource* with minimal effort on the storage layer.
