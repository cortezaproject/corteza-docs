...
[IMPORTANT]
====
The *first registered user* is automatically an *Administrator*.
====
...
If all steps complete without an explicit allow or deny, the *system default* is used, which is *deny for most cases*.
...
* **Automation**: only provides administration privileges for the automation system,
* **System admin**: only provides administration privileges for the core system.
