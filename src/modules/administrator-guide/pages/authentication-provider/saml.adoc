include::ROOT:partial$variables.adoc[]

= SAML

== Configuration

To enable SAML authentication, you need to exchange credentials from your IdP.
Configuration process involves optional key generation and setting up IdP in the Admin panel.

=== Generating private and public key pair

[NOTE]
====
This step is optional.
You can use your existing key pair for setting up SAML.
====

.Minimum required commands for generating private and public keys used for SAML authentication.
[source,shell script]
----
openssl req -x509 -sha256 -nodes -days 365 -newkey rsa:2048 \
  -keyout private.key \
  -out certificate.crt \
  -subj "/CN=corteza.your-server.com" <1>

openssl rsa -in private.key -out private-rsa.key <2>

cat certificate.crt private-rsa.key <3>
----
<1> Generates new key pairs
<2> Converts private key to RSA format
<3> Prints out both relevant keys you can copy

.Example of a public key:
[source,txt]
----
-----BEGIN CERTIFICATE-----

aDlfJubiocv/fc4BiqCt/5cPiypAmR5mSTN2x5RjLBGqLhBN
-----END CERTIFICATE-----
----

.Example of a private RSA key:
[source,txt]
----
-----BEGIN RSA PRIVATE KEY-----

XhV4F/ngKrnQgxp47BCdNe0vC5Z44OImPpvQBsZ5PueqW+Pr5JHO
-----END RSA PRIVATE KEY-----
----

=== Setting up IdP in the administration panel

Open your administration panel and navigate to System/Settings.

.SAML settings in the administration panel, System/Settings
image::authentication-provider/admin-settings.png[Screenshot of the configuration window.]

.Notes on SAML configuration form:
. If you (temporarily) disable your SAML IdP all settings are kept in Corteza.
Users will no longer be able to use SAML for login.
All active login sessions created using SAML are kept - users will not be logged-of
. Name is displayed on the login form (see below)
. Identity provider URL must point to the URL that serves provider's metadata.
Metadata is an XML document providing machine-readable instructions for SAML configuration
. Name, handle and identifier fields aid in the user creation process.
When user successfully logs-in using SAML, Corteza performs a lookup using value from identifier field. Value is expected to be an email.
Values from name and handle are used when creating a new user.


.{PRODUCT_NAME} will try to guess the *identifier* among one of:
* default identifier as defined in settings
* `emailAddress`
* `urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress`
* `urn:oasis:names:tc:SAML:attribute:subject-id`
* `email`
* `mail`

[NOTE]
====
The above values follow the industry standard, but you should consult your IdP-specific documentation on what is provided as part of user data.
====


=== Login screen with SAML enabled

image::authentication-provider/login-screen.png[Screenshot of the login screen.]

When selecting "Login with Example IdP" (or appropriate button in your scenario), user enters SAML authentication flow.
Service (Corteza) and identity (IdP) providers exchange information.
User is automatically redirected back to Corteza after successful log-in on the IdP.
