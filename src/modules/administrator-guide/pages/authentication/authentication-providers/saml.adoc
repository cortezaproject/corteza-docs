...
The assertion coming back from IdP can be signed and encrypted, providing that the certificate on the service provider side ({PRODUCT_NAME}) is included in the SAML metadata.
...
For more info on AuthNRequest and assertion in metadata refer to the <<saml-developer-notes,developer notes>>.
...
For more info on bindings in metadata refer to the <<saml-developer-notes,developer notes>>.
...
.{PRODUCT_NAME} will try to guess the identifier among one of the following:
