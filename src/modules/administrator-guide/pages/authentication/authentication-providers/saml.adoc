...
The assertion coming back from IdP can be signed and encrypted, providing that the certificate on the service provider side (Corteza) is included in the SAML metadata.
Corteza supports signed message and signed and encrypted assertions.
...
All active login sessions created using SAML are kept; users will not be logged-out
. Name is displayed on the login form (refer to the <<saml-login-ui,below screenshot>>)
