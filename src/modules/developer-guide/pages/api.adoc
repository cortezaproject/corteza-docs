...
Further development of Corteza platform outgrew that, so you may notice some peculiar patterns (returning http.HandlerFunc functions from handlers for instance)
====
...
If `corteza-server` is in the same directory as the converter tool, the files are moved to the `corteza-server/docs` directory, as well as in the /swagger directory.
...
Add a new endpoint in the corresponding `rest.yaml` file and run the `yarn convert:yaml` command in order to rebuild the definitions.

== Plans for the future

* Removing the home-brew API definitions and replacing it with OpenAPI 3.
