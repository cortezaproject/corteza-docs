...
[IMPORTANT]
====
System roles (bypass, authenticated, and anonymous) can not be re-defined at runtime.
The referenced roles should not be modifiable where the configured reference would point to a different role.
