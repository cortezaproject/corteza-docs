...
This can be as simple as allowing access to specific modules and as complex as allowing access to only specific module fields (configured in the xref:administrator-guide:federation/index.adoc#expose-module[{PRODUCT_NAME} {APP_NAME_COMPOSE} administration panel], or via the API).
