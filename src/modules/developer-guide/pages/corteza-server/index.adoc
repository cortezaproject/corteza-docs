...
[CAUTION]
====
Make sure you've properly set the `DOMAIN` `.env` variable to match your local environment, for example `DOMAIN=localhost`.
The authentication server uses this variable when generating the cookie and the authentication flow will hang if it's not defined.
