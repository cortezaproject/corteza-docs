...
[IMPORTANT]
====
Despite the 1 sentence per line rule, *free to inline your captions*.
====

Comments::
    A PlantUML comment starts with `'` (single line), or is enclosed between `/'`, and `'/`.
    Plese, use the following notation for the sake of consistency;
