...
I (the writer) use https://chromewebstore.google.com/detail/better-ruler/ilcnadaaninblgbekoaihdhoiecaflie[Better Ruler] as I've found it to be the best and simplest to use from the offerings at the time of writing.
...
To exemplify, lets remove the header and navigation from the following image.
...
To exemplify, lets only include the second subject of the following image (the top-right square).
...
To exemplify, lets focus on the Admin Area application on the app selector.
...
To exemplify, lets focus on the Admin Area application on the app selector with an annotation.
