...
Meta repositories are used for triggering build of web applications and all-in-one

|===
| repository | outputs

| corteza-webapp
| `corteza-webapp` https://hub.docker.com/r/cortezaproject/corteza-webapp[docker image] with all webapplications joined into single docker image for convenience.

| corteza
| `corteza` https://hub.docker.com/r/cortezaproject/corteza[docker image] with webapplications and sever joined into single docker image for convenience.

RPM package, placed to https://releases.cortezaproject.org/files/
...
All Corteza repositories utilize GitHub workflows/actions to release bundles and binary files and/or build docker images and other outputs.
