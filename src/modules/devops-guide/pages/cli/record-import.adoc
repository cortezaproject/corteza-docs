...
= Import {PRODUCT_NAME_COMPOSE} Records
...
.To import {PRODUCT_NAME_COMPOSE} records you need to:
...
[TIP]
====
You can use multiple definition files, such as one per module.
====
...
[NOTE]
====
Notice how we're using records from `aa.csv` to reference in `bb.csv`.
====
...
Use the xref:maintenance/cli-reference.adoc#import[import CLI command] to import your records.
