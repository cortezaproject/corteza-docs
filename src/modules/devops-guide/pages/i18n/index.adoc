...
[IMPORTANT]
====
If you update translation files after {PRODUCT_NAME} server started, you must to restart it.
...
//Editor's note: what can be enabled with the '.env' variables? Utilities for both debugging and general translations? Is that one and the same thing with 2 functions or 2 different things with 2 functions?
