...
It allows you to quickly deploy to another system and tweak how the system should behave.

Our examples are configured to work as is, but feel free to tweak xref:configuration/server.adoc[{PRODUCT_NAME} server] and xref:configuration/corredor.adoc[{APP_AUTOMATION} server] as you see fit.
...
The setup runs all services on the same network where the ports are bound to the host's network.
This is a minimum setup with MySQL as the persistent database storage with {APP_AUTOMATION} server enabled.
...
In the root of your project (next to your `docker-compose.yaml` and `.env` files), run `docker-compose up -d` (the execution can take a few seconds).

The command mentioned above (downloads and) runs all of the services configured in your `docker-compose.yaml` file.
...
This setup separates your services into two networks (internal and proxy) so that internal services are protected from the scary internet.
This is a minimum setup with MySQL as the persistent database storage with {APP_AUTOMATION} server enabled.
...
Inside your `my-corteza` directory (next to your `docker-compose.yaml` and `.env` files), run `docker-compose up -d` (the execution can take a few seconds).
