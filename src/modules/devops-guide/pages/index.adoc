...
= DevOps guide
:description: Learn how to setup and maintain the {PRODUCT_NAME} ecosystem
:keywords: dev-ops, releasing, maintenance, devops
...
image::arch-overview.png[{PRODUCT_NAME} architectural overview,role="data-zoomable"]
...
Offline deploy setups run all services on the same network where the ports are bound to the host's network.
