...
This part define https://datatracker.ietf.org/doc/html/rfc2616#section-13.5.1[hop-by-hop] headers to be passed from a client to proxied server.
This will allow websockets proxying to works with your nginx configuration above.

[IMPORTANT]
====
The complete guide is available on https://nginx.org/en/docs/http/websocket.html[nginx documentation website] and provide much more information and examples.
====

Inside your `nginx.conf` file (default placed under `/etc/nginx` directory) add these following lines inside your "server" configuration section.
