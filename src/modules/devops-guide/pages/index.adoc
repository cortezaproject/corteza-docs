...
📁 my-corteza
  ...
  📁 data <1>
    ...
    📁 db <3>
----
<1> The images take care of the ownership of these directories on first start; you only need to `chown` them yourself if you run the containers as a non-root user.
...
<3> The images take care of the ownership of these directories on first start; you only need to `chown` them yourself if you run the containers as a non-root user.
Omit if you won't use persistent storage.
