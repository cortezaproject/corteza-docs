...
[NOTE]
====
Refer to more docker compose examples at xref:devops-guide:index.adoc#useful-commands[useful Docker commands].
====
...
First, find the container ID of the running server in the current project directory.
After the container ID is found, use it to execute the CLI command.
For the illustration purpose, we will name the service name `server`.
