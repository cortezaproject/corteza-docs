...
To exemplify; the `/test-endpoint` endpoint for the `https://latest.cortezaproject.org` instance is available under `https://latest.cortezaproject.org/api/gateway/test-endpoint`
...
Request validation is done using *prefilters*
A prefilters allow you to validate the request and determine if the given endpoint should process it.
