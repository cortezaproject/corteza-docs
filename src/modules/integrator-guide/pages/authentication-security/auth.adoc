...
You can use our https://github.com/cortezaproject/corteza-vue/blob/2021.6.x/src/plugins/auth.ts[corteza-vue plugins/auth] to handle this for you.
====
...
Please see xref:devops-guide:configuration/server.adoc#_authentication[full list of options].
These changes take affect after server restart.

=== Notes for debugging

.Unordered list of things to check when authentication is not working:
