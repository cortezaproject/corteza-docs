...
|
The automation script is *terminated* when an error other then `Aborted` occurs.
...
|
The automation script is *aborted* when an `Aborted` error occurs, or the automation script returned a `false` value.
This normaly causes the current automation script to terminate, but permits the original operation to continue.
...
`istanbul ignore next` excludes the following function from the coverage relay.
====
