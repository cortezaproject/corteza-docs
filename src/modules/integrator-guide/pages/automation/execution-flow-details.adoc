...
[plantuml,exec-flow-dig-implicit,svg,role=exec-flow-dig-implicit]
----
@startuml
actor User as u
...
[NOTE]
====
* *DevNote* what about the `Abort` we had with automation scripts*
* *DevNote* can the resulting value be zero with workflows?
====
...
[TIP]
====
In some cases, you can use explicit automation to replace the need for sink automation.
====
...
[plantuml,exec-flow-dig-explicit,svg,role=exec-flow-dig-explicit]
----
@startuml
actor User as u
participant "Corteza component" as cc
...
[plantuml,exec-flow-dig-deferred,svg,role=exec-flow-dig-deferred]
----
@startuml
...
[plantuml,exec-flow-dig-sink,svg,role=exec-flow-dig-sink]
----
@startuml
