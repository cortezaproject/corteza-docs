...
{PRODUCT_NAME} provides a powerful automation system to implement custom business logic.
...
[TIP]
====
Our https://github.com/cortezaproject/corteza-ext/blob/2021.3.x/crm/config/1600_workflows.yaml[CRM] and https://github.com/cortezaproject/corteza-ext/blob/2021.3.x/service-solution/config/1600_workflows.yaml[Service Solution] applications are also open-source.
...
[IMPORTANT]
====
Please read this sub-section carefully as it outlines some system design decisions that may not work as expected if assumed or not used properly.
====
...
If you need to explicitly update the same resource, define some flags to help you keep track of what is going on.
...
[#inf-exec-cycle]
.Diagram outlining the potential infinite execution cycle. Note how the explicit update loops back to the update request step.
[plantuml,inf-exec-cycle,svg,role=inf-exec-cycle]
----
@startuml
...
For example; an asynchronous automation can not change the values of the record that is about to be created or updated.

[CAUTION]
====
Any workflow that defines a prompt or a delay step is executed asynchronously as the system can not guarantee that the execution will be finished in a timely manner, causing further complications and confusion.
====

== Access control

When an automation is executed and the automation interacts with {PRODUCT_NAME}, the user that *caused the automation to execute* (the invoking user) is used when performing access control checks.

If the invoking user is allowed to access some resource, the automation is also allowed to access that resource.
If the invoking user is *not* allowed to access some resource, the automation is also *not* allowed to access that resource.
...
{PRODUCT_NAME} provides two facilities for defining automation: xref:automation/workflows/index.adoc[workflows] and xref:automation/automation-scripts/index.adoc[automation scripts].

[IMPORTANT]
====
As of xref:ROOT:changelog/202103.adoc[2021.3], with the introduction of workflows, automation scripts are disabled by default and can be enabled by setting the xref:devops-guide:configuration/server.adoc#_corredor_enabled[`CORREDOR_ENABLED=true`] `.env` variable.
====

A xref:automation/workflows/index.adoc[workflow] is presented as a simplified BPMN diagram, allowing you to configure instead of code.
Workflows are easier to visualize and maintain, so they should be used whenever possible.
If your automation requirement is on a larger scale with operations that are not supported by the workflow engine, an automation script is a better choice.

[TIP]
====
A workflow can xref:automation/workflows/automation-scripts.adoc[execute automation scripts] allowing you to mix-and-match the two.
====
...
You can enable the xref:devops-guide_/_configuration/corredor.adoc#CORREDOR_EXT_SERVER_SCRIPTS_WATCH[`CORREDOR_EXT_SERVER_SCRIPTS_WATCH`] and xref:devops-guide_/_configuration/corredor.adoc#CORREDOR_EXT_CLIENT_SCRIPTS_WATCH[`CORREDOR_EXT_CLIENT_SCRIPTS_WATCH`] `.env` variables to watch for changes.
====

[#internals-overview]
.The diagram provides the general idea of how the different components interact and how a request invokes an automation. Refer to the xref:automation/execution-flow-details.adoc[execution flow details] section for a deeper dive into the internals.
[plantuml,internals-overview,svg,role=internals-overview]
----
@startuml
actor User as u
...
[NOTE]
====
The provided data and metadata depends on the trigger.
...
The provided data may be used to control the automation execution, and in some cases even manipulate the original operation.
...
[CAUTION]
====
An automation will inherit the permissions of the *invoking user* (the user that caused the automation to execute).
A workflow that attempts to perform operations (such as accessing sensitive data), to which the invoking user does not have sufficient permissions, will fail.

The invoking user may be overwritten by explicitly setting the security context (the *`runAs`* parameter).
====

When the automation is finished executing, it returns an *execution result*.
The execution result _may_ dictate on how the initial operation should resolve.

.A quick outline of how the execution result affects the initial operation:
* If the automation is asynchronous, the execution result is *ignored* and does not manipulate the initial operation (it is ignored).
* If the automation results in an *unknown error*, the initial operation is canceled and no additional processing is done.
* If the automation results in an *aborted error*, the initial operation is allowed to continue and execute additional automation.
...
{PRODUCT_NAME} components dispatch events on the event bus when the system is about to interact with the components' resource.
This allows us to detect specific operations and build upon the standard flow, such as create audit log entries, or send a welcome email when a user signs up.
We can also cancel the operation by raising errors when something is not ok.

We can react to such events, by defining a `before`/`after` event listener for a system resource.
For example; we would bind the `beforeCreate` event for `Compose Record` resource.
We label such triggers as *implicit* where we also use the same label for the corresponding automation.
...
[TIP]
====
https://crontab.guru/[Crontab guru] is a tool to help you write and debug your cron expressions.
====

The default granularity for timestamps and intervals is *one minute*.
You can change this by setting the xref:devops-guide:configuration/server.adoc#_eventbus_scheduler_interval[`EVENTBUS_SCHEDULER_INTERVAL`] `.env` variable.

[CAUTION]
====
Finer granularity may lower the systems' performance.
====
...
For example; we would define a sink route with the path of `POST /system/sink/api/request-demo` that would collect potential leads sent over by our landing page.
...
React to email requests by defining an event on the `System Mail` resource; either `onReceive` for inbound emails, or `onSend` for outbound emails.
...
An explicit automation is usually represented as a button on of of the {PRODUCT_NAME} user interfaces.

[NOTE]
====
Explicit automation can be defined or specific resources (such as records and users) which indicates the initial scope (execution arguments) of the automation.
====
...
To go into more details regarding the execution flow and what happens under-the-hood, refer to the xref:automation/execution-flow-details.adoc[execution flow details section].
...
To see some actual examples, refer to out https://github.com/cortezaproject/corteza-ext/blob/2021.3.x/crm/config/1600_workflows.yaml[CRM] and https://github.com/cortezaproject/corteza-ext/blob/2021.3.x/service-solution/config/1600_workflows.yaml[Service Solution] applications.
