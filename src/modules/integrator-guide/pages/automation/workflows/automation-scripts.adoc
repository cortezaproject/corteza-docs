...
export default {
  ...
  triggers ({ on }) {
    /**
     * Due to how the Corredor scripting system is designed right now, triggers still need to be defined (even if the script is executed explicitly from the workflow).
     */
...
//Editor's note: some string in line 67 does not work if it is not a name of a string - try "any string" or elaborate further if that does not work, so I can find sth that does work. Thank you.
...
If the automation script returns a `false` string, we consider this an `Abort` signal and the workflow results with an error.
====
