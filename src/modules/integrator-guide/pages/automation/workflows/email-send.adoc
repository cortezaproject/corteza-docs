...
In case you appoint the email function without specifying the `from` argument, the `SMTP_FROM` `.env` variable is used.
...
//Editor's note: Elaborate what you mean in line 20; the source code is as it follows, an example of the source code can be found [here-link]...
...
//Editor's note: initialize seems odd and too specific to business/computing English - if assemble fits, then great, otherwise please find a different word or elaborate so I can.
...
//Editor's note: Elaborate what you mean in line 96; the source code is as it follows, an example of the source code can be found [here-link]. 
...
//Editor's note: The function overwrites the email headers provided by whom with ones provided by whom?
...
//Editor's note: The function overwrites the email headers provided by whom with ones provided by whom?
//Editor's note: is email header and email headers the same ooor...? In case it is the same, keep the singular one, otherwise explain the difference.
