...
[#iterator-kind-condition]#<<iterator-kind-condition,Condition iterators>>#::
  ...
  To exemplify; a condition of `counter < 10` would make the iterator repeat for as long as the counter is below 10.
...
[#iterator-kind-item]#<<iterator-kind-item,Item iterators>>#::
  ...
  To exemplify; we pass in 10 records, therefore the iterator repeats 10 times, once for each record in the provided set.
