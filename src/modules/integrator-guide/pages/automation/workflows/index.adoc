...
= Workflows
:description: {PRODUCT_NAME} workflow allows you to implement custom business logic without the need for any programming knowledge.
...
//Editor's note: Add an image description
...
//Editor's note: Clarify what is meant by "out if the box" - outside of a certain menu, section...?
...
//Editor's note: Specify what "out of the box" means here
...
|
A *termination step* image:workflow/termination.svg[width=20px] terminates the execution of a workflow.
...
//Editor's note: only keep the capital S in spreadsheets if that is a title/subtitle/name of sth...
...
// check vv
// Use expressions when the passed data should change based on the workflows' scope.
//Editor's note: You talk about how the Iterator arguments can be passed in 3 different ways yet mention only 2!
...
|
An *error step* image:workflow/error.svg[width=20px] allows you to manually trigger an error during the execution of a workflow.
