...
//Editor's note: Primitive or not?
...
| Yes
|
A `String` variable represents a string.
//Editor's note: Check for accuracy.
...
//Editor's note: ... is a rendered template created by the usage of RenderOptions and RenderVariables/... is a rendered template. It uses RenderOptions and RenderVariables in order to produce a document. 
// --> choose/restructure the original so it is clear what the verb "use" pertains to.
...
//Editor's note: should not or cannot?
