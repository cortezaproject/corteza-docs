...
The tool is currently only available in the command line interface (CLI) and supports synthetic data generation for {APP_NAME_COMPOSE} records (`compose records compose records`) and users (`users synthetic`)

Generated records are flagged with `synthetic:true` (meta), and users will have their handles prefixed with "synthetic_" and email domain "synthetic.tld".
