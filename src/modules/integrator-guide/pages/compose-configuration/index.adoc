...
[IMPORTANT]
====
In case your module does not define a record page, you are not be able to create or update its records through the user interface.

You are still able to view its records, create the records using inline record lists, and create records using xref:automation/index.adoc[automation].
====
...
//Editor's note: general purpure? Couldn't find what purpure is. Check the spelling, maybe I missed sth
...
You may only use numeric fields as dimension fields.
A `count` field returns the number of records that match the configuration parameters.
...
Validators can not change their value.
...
The flexible access control system (RBAC) allows you to either allow or deny access to all resources except for records (we will add this feature in the future).
