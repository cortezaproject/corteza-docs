...
[NOTE]
====
As of 2024.9.x, {PRODUCT_NAME} introduces user groups which you can use to implement hierarchical access.
====
...
[IMPORTANT]
====
.The two components are merged using the following check-list.
. If user-assigned roles result in explicit deny, the result is deny.
. If user-assigned roles result in explicit allow, the result is allow.
. If user group hierarchy result in explicit allow, the result is allow.
. If user group hierarchy result in explicit deny, the result is deny.
. The result is inherit.
====

=== Organization tree branch evaluation

{PRODUCT_NAME} determines the organization tree branch where the requesting user's is at the very top.
{PRODUCT_NAME} evaluates each node in the branch following the standard role evaluation.

The evaluation results in *allow* if at least one node results in *allow* (allow-first).
