...
Refer to xref:workflows/types.adoc[type index] and the xref:workflows/type-ref.adoc[type reference] for more details.
====
...
Adding some extra steps may help you get the feel for the interface.
Use expressions image:workflow/expressions.svg[width=20px] to change values (such as the cost of a lead) and functions image:workflow/function.svg[width=20px] to perform operations such as creating or updating records.
...
To indicate where the workflow starts, you should use a trigger image:workflow/trigger.svg[width=20px].
A trigger defines the timing of the execution of a specific workflow.
If you want the workflow to run when a new record is created, you simply define a trigger with the appropriate event.
...
To indicate where the workflow ends, you should use a termination step image:workflow/termination.svg[width=20px].
The termination step terminates the execution regardless what stage the workfow is in; the beginning, middle or end.
...
There are two kinds of steps to interact with values in the workflow; expressions image:workflow/expressions.svg[width=20px] and functions image:workflow/function.svg[width=20px].
...
Refer to xref:expressions/index.adoc[] for more details regarding expression definition.

[NOTE]
====
Think of an expression step as a block of code that interacts with variables.
====

.Examples of usage:
* generate a full name for a contact,
// Nika's note: "this" is too broad as it may reffer to various things. Specify what "this" is.
...
A *function step* image:workflow/function.svg[width=20px] takes in the values defined within a workflow, performs some operations and generates a result where that is relevant.
A function can interact with {PRODUCT_NAME} or a supported external service.

[NOTE]
====
Think of the function step as calling a built-in function.
====
