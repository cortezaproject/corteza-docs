...
A swimlane image:workflow/swimlane.svg[width=20px] is a purely visual element and does not affect the actual execution of the workflow.
In smaller workflows that really perform a single operation (such as notifying administrator about a client sign-up or logging changes) swimlanes don't make much sense and may even reduce the clarity.
For larger projects however, swimlanes can be a neat addition as they help you visually group smaller steps into larger operations and thus allowing you to understand what a specific part of the workflow does.

== Example

Lets firstly build a basic example.
...
Another neat feature would be to track the changes using an audit log.
To do this, we need to create a new record for the audit log module.
