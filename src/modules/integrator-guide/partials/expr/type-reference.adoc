...
A `Boolean` indicates whether a variable is truthy (`true`) or faulty (`false`).
...
[NOTE]
====
You can use any value parsable by Golang's https://pkg.go.dev/time#ParseDuration[ParseDuration function].
====
...
[NOTE]
====
You may use multiple <<duration-units,duration units>> as long as they are in a descending order.

* A valid example: `10m5s2ms`,
* Invalid example: `2ms5s10m`.
====
